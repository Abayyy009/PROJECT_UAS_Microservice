						...
						<form method="post" action="checkout_act.php">
							...
							<div class="col-lg-6">
								...
								if (isset($_GET['id']) && isset($_GET['jumlah'])) {
									// Retrieve product data using product ID
									$produk_id = htmlspecialchars($_GET['id']);
									$jumlah = htmlspecialchars($_GET['jumlah']);

									// Assume you have a PDO connection $conn
									$stmt = $conn->prepare("SELECT * FROM produk WHERE produk_id = :produk_id");
									$stmt->execute([':produk_id' => $produk_id]);
									$produk = $stmt->fetch(PDO::FETCH_ASSOC);

									if ($produk) {
										$total_harga = $produk['produk_harga'] * $jumlah;
										$total_berat = $produk['produk_berat'] * $jumlah;
										?>
										<table class="table table-bordered">
											...
											<tbody>
												<tr>
													<td>
														<a href="produk_detail.php?id=<?php echo $produk['produk_id']; ?>">
													...
													<td class="text-center">
														<strong class="primary-color total_harga">
											...
											<tfoot>
												...
												<tr>
													...
													<th class="text-center">
														<span
															id="tampil_total"><?php echo "Rp. " . number_format($total_harga) . " ,-"; ?></span>
													</th>
												</tr>
											</tfoot>
										</table>
										...
										echo "<br><br><br><h3><center>Produk tidak ditemukan. Silakan <a href='index.php'>kembali berbelanja</a>.</center></h3><br><br><br>";
									}
								} else {
...
<!-- Add jQuery and AJAX script to handle province and kabupaten selection -->
